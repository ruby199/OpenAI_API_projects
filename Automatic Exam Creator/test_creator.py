...
class TestGenerator:
    ...
    def run(self):
        ...
        if TestGenerator._verify_prompt(prompt):
            respose = self.generate_quiz(prompt)
    ...
    def creawte_prompt(self):
        prompt = f"Create a multiple choice quiz on the topic of {self.topic} consisting of {self.num_questions} "\
        + f"Each question sould have {self.num_possible_answers} options. "\
